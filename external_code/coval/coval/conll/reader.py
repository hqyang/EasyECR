...
from external_code.coval.coval.conll import mention
...
def get_coref_infos_from_clusters(doc_pred_clusters: Dict[str, List[str]],
                                  doc_glod_clusters: Dict[str, List[str]],
                                  keep_singletons: bool):
    """

    :param doc_pred_clusters:
    :param doc_glod_clusters:
    :param keep_singletons:
    :return:
    """
    doc_coref_infos = {}

    for doc, gold_clusters in doc_glod_clusters.items():
        if not keep_singletons:
            gold_clusters = [e for e in gold_clusters if len(e) > 1]

        pred_clusters = doc_pred_clusters[doc]
        if not keep_singletons:
            pred_clusters = [e for e in pred_clusters if len(e) > 1]
        ...
        doc_coref_infos[doc] = (gold_clusters, pred_clusters,
                key_mention_sys_cluster, sys_mention_key_cluster)
