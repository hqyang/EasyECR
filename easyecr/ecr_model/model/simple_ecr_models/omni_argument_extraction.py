...
from transformers import BertTokenizerFast, RobertaTokenizerFast, T5TokenizerFast, MT5TokenizerFast, BartTokenizerFast
...
class EDProcessor:
    ...
    def tokenize_per_instance(self, text, schema):
        ...
        input_context = self.tokenizer(
            words, truncation=True, padding="max_length", max_length=self.max_seq_length, is_split_into_words=True
        )
        return dict(
...
def get_ed_result(texts, triggers):
    ...
    for i, text in enumerate(texts):
        ...
        for trigger in triggers_in_text:
            ...
            event = {"type": type, "trigger": mention, "offset": offset}
            events.append(event)
        results.append({"text": text, "events": events})
    return results
    ...
    for i, instance in enumerate(instances):
        ...
        for trigger, argus_in_trigger in zip(instance["triggers"], arguments):
            ...
            for argu in argus_in_trigger:
                ...
                argument = {"mention": mention, "offset": offset, "role": role}
        ...
        results.append({"text": instance["text"], "events": events})
    return results
...
def prepare_for_eae_from_input(texts, all_triggers, schemas):
    ...
    for text, triggers, schema in zip(texts, all_triggers, schemas):
        instance = {"text": text, "schema": schema, "triggers": []}
        for trigger in triggers:
            instance["triggers"].append({"mention": trigger[0], "offset": [trigger[1], trigger[2]]})
...
def prepare_for_eae_from_pred(texts, triggers, schemas):
    ...
    for i, text in enumerate(texts):
        triggers_in_text = [trigger for trigger in triggers if trigger[0] == i]
        instance = {"text": text, "schema": schemas[i], "triggers": []}
        for trigger in triggers_in_text:
            type = trigger[1]
            mention = trigger[2]
            if mention not in text:
                continue
            offset = find_position(mention, text)
            instance["triggers"].append({"type": type, "mention": mention, "offset": offset})
        instances.append(instance)
    return instances
...
def generate(model, tokenizer, inputs):
    gen_kwargs = {"max_length": 128, "num_beams": 4, "synced_gpus": False, "prefix_allowed_tokens_fn": None}
...
def get_pretrained(path, device):
    ...
    # model
    model_args = AttrDict({"paradigm": "seq2seq", "model_type": "mt5"})
...
if __name__ == "__main__":
                    ...
                    if create_segment:
                        ...
                        if sent_idx > 0 and sentences_lengths[sent_idx - 1] + sentences_lengths[sent_idx] < 512:
                            segment_mapping.append(
                                {
                                    "new_start": 0,
                                    "new_end": len(sentences[sent_idx - 1]["text"]),
                                    "start": sentences[sent_idx - 1]["start"],
                                }
                            )
                        ...
                        segment_mapping.append(
                            {
                                "new_start": len(new_segment),
                                "new_end": len(new_segment) + len(sentences[sent_idx]["text"]),
                                "start": sentences[sent_idx]["start"],
                            }
                        )
                        ...
                        if sent_idx < len(sentences) - 1 and total_length + sentences_lengths[sent_idx + 1] < 512:
                            segment_mapping.append(
                                {
                                    "new_start": len(new_segment),
                                    "new_end": len(new_segment) + len(sentences[sent_idx + 1]["text"]),
                                    "start": sentences[sent_idx + 1]["start"],
                                }
                            )
                            new_segment += sentences[sent_idx + 1]["text"]
                        ...
                        for event in results:
                            ...
                            for mapping in segment_mapping:
                                if (
                                    event["offset"][0] >= mapping["new_start"]
                                    and event["offset"][1] <= mapping["new_end"]
                                ):
                ...
                Data.append(
                    {
                        ...
                        "true_label": [
                            ...
                            if event["subtype"] in CATEGORIES
                        ],
                    }
                )
    ...
    def evalute_ed(data_file):
        ...
        tp_avg, fp_avg, fn_avg = 0.0, 0.0, 0.0
        ...
        with open(event_file, "rt", encoding="utf-8") as f:
            for line in tqdm(f.readlines()):
                ...
                for sent in sentences:
                    sent["event"] = []
                for event in sample["events"]:
                    sentences[event["sent_idx"]]["event"].append(
                        (event["trigger"], event["sent_start"], event["sent_start"] + len(event["trigger"]))
                    )
                for sent in sentences:
                    if not sent["event"]:
                        continue
                ...
                Data.append({"doc_id": sample["doc_id"], "document": sample["document"], "event_args": pred_event_args})
        Data.sort(key=lambda x: x["doc_id"])
        ...
        with open(pred_event_file, "rt", encoding="utf-8") as f:
            for line in tqdm(f.readlines()):
                ...
                for event in sample["pred_label"]:
                    sent_idx, sent_start = get_event_sent_idx(
                        event["start"], event["start"] + len(event["trigger"]) - 1, sentences
                    )
                    sentences[sent_idx]["event"].append(
                        (event["trigger"], sent_start, sent_start + len(event["trigger"]))
                    )
                for sent in sentences:
                    if not sent["event"]:
                        continue
    ...
    event_argument_extraction(
        "/home/nobody/code/prompt-event-coref-emnlp2023/data/train_filtered.json",
        "./argument_files/omni_train_pred_args.json",
    )
    # event_argument_extraction(
    #     "/home/nobody/code/prompt-event-coref-emnlp2023/data/dev_filtered.json",
    #     "./argument_files/omni_dev_pred_args.json",
    # )
    # event_argument_extraction(
    #     "/home/nobody/code/prompt-event-coref-emnlp2023/data/test_filtered.json",
    #     "./argument_files/omni_gold_test_pred_args.json",
    # )
    # event_argument_extraction_for_testfile(
    #     "/home/nobody/code/prompt-event-coref-emnlp2023/data/test_filtered.json",
    #     "../epoch_3_test_pred_events.json",
    #     "./argument_files/omni_epoch_3_test_pred_args.json",
    # )
