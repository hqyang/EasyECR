...
def create_event_context(
    ...
    sentence_lens: Optional[list] = None,
) -> dict:
    if "kbp" in dataset_name:
        if e1_sent_idx == e2_sent_idx:  # two events in the same sentence
            ...
            if total_length > max_length:  # cut segment
                ...
                core_context_before = tokenizer.decode(
                    tokenizer.encode(core_context_before)[1:-1][-before_after_length:]
                )
    ...
    else:  # mavenere ace2005数据集的适配
        e1_core_context_before = f"{e1_extent[:e1_sent_start]}"
        e1_core_context_after = f"{e1_extent[e1_sent_start + len(e1_trigger):]}"
        e1s_offset = 0
        e1_core_context_middle = f"{s_tokens['e1s']} {e1_trigger} "
        e1e_offset = len(e1_core_context_middle)
        e1_core_context_middle += f"{s_tokens['e1e']}"
        # e2 source sentence
        e2_core_context_before = f"{e2_extent[:e2_sent_start]}"
        e2_core_context_after = f"{e2_extent[e2_sent_start + len(e2_trigger):]}"
        e2s_offset = 0
        e2_core_context_middle = f"{s_tokens['e2s']} {e2_trigger} "
        e2e_offset = len(e2_core_context_middle)
        e2_core_context_middle += f"{s_tokens['e2e']}"
...
def create_prompt(
    dataset_name: str,
    e1_sent_idx: Optional[int],
    e1_sent_start: int,
    e1_trigger: str,
    e1_extent: str,
    e1_related_info: Optional[dict],
    e2_sent_idx: Optional[int],
    e2_sent_start: int,
    e2_trigger: str,
    e2_extent: str,
    e2_related_info: Optional[dict],
    prompt_type: str,
    ...
    if prompt_type.startswith("h") or prompt_type.startswith("s"):  # base prompt
        ...
        context_data = create_event_context(
            ...
            e1_sent_idx=e1_sent_idx,
            e1_sent_start=e1_sent_start,
            e1_trigger=e1_trigger,
            e1_extent=e1_extent,
            e2_sent_idx=e2_sent_idx,
            e2_sent_start=e2_sent_start,
            e2_trigger=e2_trigger,
            e2_extent=e2_extent,
            sentences=sentences,
            sentence_lens=sentence_lens,
            s_tokens=special_token_dict,
            tokenizer=tokenizer,
            max_length=max_length - template_length,
        )
...
class CorefPromptCoreferenceDataset(Dataset):
    def __init__(
        ...
        self.dataset_name = ecr_data.name
        ...
        # print("当前数据集包含的文档ID：", list(self.documents.keys()))
        # print("当前数据集的长度：", len(list(self.documents.keys())))
        # print("当前数据集的相关字典包含的文档ID：", list(self.related_dict.keys()))
        # print("当前数据集的相关字典的文档长度：", len(list(self.related_dict.keys())))

        # print(set(self.documents.keys()) & set(self.related_dict.keys()))
        # print(len(set(self.documents.keys()) & set(self.related_dict.keys())))
    ...
    def data_sampling(self):
        ...
        for d in self.data:
            if d["label"] == 1:
                ...
                neg_data.append(d)
        pl = len(pos_data)
        neg_data = random.sample(neg_data, pl)
    ...
    def adapt_data(self):
        ...
        for doc_id, doc in self.documents.items():
            ...
            if self.dataset_name == "kbpmix":
                sentences = doc.meta["sentences"]
                sentences_lengths = [len(self.tokenizer.tokenize(sent["text"])) for sent in sentences]
            elif self.dataset_name == "ace2005eng":
                sentences = None
                sentences_lengths = None
            elif self.dataset_name == "mavenere":
                sentences = doc.meta["sentences"]
                sentences_lengths = [len(self.tokenizer.tokenize(sent)) for sent in sentences]
            # create event pairs
            for i in range(len(doc_mentions) - 1):
                for j in range(i + 1, len(doc_mentions)):
                    ...
                    event_1_related_info, event_2_related_info = None,None
                    # event_1_related_info = self.related_dict[doc_id][event_1.anchor.start]
                    # event_2_related_info = self.related_dict[doc_id][event_2.anchor.start]
                    if self.dataset_name == "kbpmix":
                        event_1_sent_idx = event_1.meta["sent_idx"]
                        event_2_sent_idx = event_2.meta["sent_idx"]
                        event_1_subtype = event_1.meta["subtype"]
                        event_2_subtype = event_2.meta["subtype"]
                        e1_sent_start = event_1.anchor.start - event_1.extent.start
                        e2_sent_start = event_2.anchor.start - event_2.extent.start
                    elif self.dataset_name == "mavenere":
                        event_1_sent_idx = event_1.meta["sent_idx"]
                        event_2_sent_idx = event_2.meta["sent_idx"]
                        event_1_subtype = event_1.meta["event_type"]
                        event_2_subtype = event_2.meta["event_type"]

                        event_1_offset, event_2_offset = 0, 0
                        for sent in sentences[:event_1_sent_idx]:
                            event_1_offset += len(sent)
                        for sent in sentences[:event_2_sent_idx]:
                            event_2_offset += len(sent)
                        sent1_l = []
                        for item in doc.meta["doc_token"]:
                            if item["sentence"] == event_1_sent_idx:
                                sent1_l.append(item["text"])
                        sent2_l = []
                        for item in doc.meta["doc_token"]:
                            if item["sentence"] == event_2_sent_idx:
                                sent2_l.append(item["text"])

                        e1_sent_start = 0
                        for w in sent1_l[: event_1.anchor.start - event_1.extent.start]:
                            e1_sent_start += len(w)
                        e2_sent_start = 0
                        for w in sent2_l[: event_2.anchor.start - event_2.extent.start]:
                            e2_sent_start += len(w)
                    else:
                        event_1_sent_idx, event_2_sent_idx = None, None
                        event_1_subtype = event_1.meta["subtype"]
                        event_2_subtype = event_2.meta["subtype"]
                        e1_sent_start = event_1.anchor.start - event_1.extent.start
                        e2_sent_start = event_2.anchor.start - event_2.extent.start
                    ...
                    prompt_data = create_prompt(
                        ...
                        e1_sent_idx=event_1_sent_idx,
                        e1_sent_start=e1_sent_start,
                        e1_trigger=event_1.anchor.text,
                        e1_extent=event_1.extent.text,
                        e1_related_info=event_1_related_info,
                        e2_sent_idx=event_2_sent_idx,
                        e2_sent_start=e2_sent_start,
                        e2_extent=event_2.extent.text,
                        e2_trigger=event_2.anchor.text,
                        e2_related_info=event_2_related_info,
                        sentences=sentences,
                        sentence_lens=sentences_lengths,
                        prompt_type=self.prompt_type,
                        select_arg_strategy=self.select_arg_strategy,
                    ...
                    data.append(
                        {
                            ...
                            "e1_id": event_1.anchor.start,  # event1
                            "e1_trigger": event_1.anchor.text,
                            "e1_subtype": event_1_subtype if event_1_subtype in EVENT_SUBTYPES else "normal",
                            "e1_subtype_id": subtype2id.get(event_1_subtype, 0),  # 0 - 'other'
                            ...
                            "e2_id": event_2.anchor.start,  # event2
                            "e2_trigger": event_2.anchor.text,
                            "e2_subtype": event_2_subtype if event_2_subtype in EVENT_SUBTYPES else "normal",
                            "e2_subtype_id": subtype2id.get(event_2_subtype, 0),  # 0 - 'other'
...
class CorefPromptModel(PlEcrModel):
    ...
    def collate_fn(self, batch_samples):
        ...
        for sample in batch_samples:
            ...
            idxs = []
            for offset in [sample["e1s_offset"], sample["e1e_offset"], sample["e2s_offset"], sample["e2e_offset"]]:
                idx = encoding.char_to_token(offset)
                while idx == None:
                    offset -= 1
                    idx = encoding.char_to_token(offset)
                idxs.append(idx)

            e1s_idx, e1e_idx, e2s_idx, e2e_idx = idxs
    ...
    def prepare_data(self, data: EcrData, mode: str):
        if data is None:
            return None, None
        if mode == "train":
            if len(data.documents) > 100 or len(data.documents) == 1:
...
class CorefPromptEcrData:
    ...
    def get_pred_arguments(self):
        ...
        arg_dict = {}
        with open(self.arg_file, "rt", encoding="utf-8") as f:
            for line in f:
                sample = json.loads(line.strip())
                arg_dict[sample["doc_id"]] = {
                    event["start"]: [
                        ...
                        if arg["role"].lower() in participant_roles | place_roles
                    ]
                    for event in sample["event_args"]
                }
        return arg_dict

    def get_simi_events(self):
        doc_simi_dict = {}
        for doc_id, doc in self.documents.items():
            if not doc.meta.get("event_pairs_id") and not doc.meta.get("event_pairs_cos"):
                continue
            event_pairs_id, event_pairs_cos = doc.meta["event_pairs_id"], doc.meta["event_pairs_cos"]
            simi_dict = defaultdict(list)
            for id_pair, cos in zip(event_pairs_id, event_pairs_cos):
            ...
            for simi_list in simi_dict.values():
                simi_list.sort(key=lambda x: x["cos"], reverse=True)
            doc_simi_dict[doc_id] = simi_dict
        return doc_simi_dict

    def create_simi_event(self, data: EcrData, arg_file: str, save_file: str):
        self.data = data
        ...
        self.doc_arg_dict = self.get_pred_arguments()
        self.doc_simi_dict = self.get_simi_events()
        ...
        for doc_id, doc in self.documents.items():
            simi_info = {}
            if not self.doc_arg_dict.get(doc_id) or not self.doc_simi_dict.get(doc_id):
                Results.append({"doc_id": doc_id, "relate_info": simi_info})
                continue
            arg_dict, simi_dict = self.doc_arg_dict[doc_id], self.doc_simi_dict[doc_id]
            mentions = doc2mentions[doc_id]
            for m in mentions:
                if m.mention_id in simi_dict:
                    triggers, trigger_offsets, args = set(), [], []
                    for related_m in simi_dict[m.mention_id]:
                        r_m = self.mentions[related_m["id"]]
                        triggers.add(r_m.anchor.text)
                        trigger_offsets.append(r_m.anchor.start)
                        args += arg_dict[r_m.anchor.start]
                    simi_info[m.anchor.start] = {
                        "trigger": m.anchor.text,
                        "arguments": arg_dict[m.anchor.start],
            ...
            Results.append({"doc_id": doc_id, "relate_info": simi_info})
        with open(self.save_file, "wt", encoding="utf-8") as f:
